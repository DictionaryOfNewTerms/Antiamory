...
| **Token Variable** | `ANTIAMORY_GITHUB_TOKEN` |
| **Username Variable** | `ANTIAMORY_GITHUB_USERNAME` |
| **Credential File** | `~/.env.credentials/antiamory.env` |
...
```bash
echo $ANTIAMORY_GITHUB_TOKEN  # Should show token (not NOT_CONFIGURED)
...
| Setting | Value |
|---------|-------|
| Token Variable | `ANTIAMORY_[SERVICE]_TOKEN` |
| Project ID | `xxx` |
...
vercel --prod --token $ANTIAMORY_VERCEL_TOKEN
```
